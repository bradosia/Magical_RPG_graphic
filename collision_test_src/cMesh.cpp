...
cMeshEntry::cMeshEntry(aiMesh* mesh) {
	...
	if (mesh->HasPositions()) {
		vertices1D = new float[nbVertices * 3];
		vertices2D = new float*[nbVertices];
		...
		glBufferData(GL_ARRAY_BUFFER, 3 * nbVertices * sizeof(GLfloat),
	...
	if (mesh->HasTextureCoords(0)) {
		float *texCoords = new float[nbVertices * 2];
		for (unsigned int i = 0; i < nbVertices; ++i) {
		...
		glBufferData(GL_ARRAY_BUFFER, 2 * nbVertices * sizeof(GLfloat),
	...
	if (mesh->HasNormals()) {
		...
		glBufferData(GL_ARRAY_BUFFER, 3 * nbVertices * sizeof(GLfloat), normals,
				GL_STATIC_DRAW);
	...
	if (mesh->HasFaces()) {
		unsigned int *indices = new unsigned int[nbFaces * 3];
		faceIndexes1D = new int[nbFaces * 3];
		faceIndexes2D = new int*[nbFaces];
		...
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, 3 * nbFaces * sizeof(GLuint),
